...
This project introduces **Adaptive Feedback-Aware Modified Dijkstra**, a novel routing algorithm that learns from historical route performance without machine learning.

**Key Innovation:**
- Edge weights are **dynamic** and learn from feedback
- After each completed route, edge statistics are updated
- Future routes benefit from accumulated historical data
- System converges to better solutions over time

**Why This Is Novel:**
- Standard weighted graphs use **static** edge weights
- Our approach uses **adaptive** weights that change based on empirical data
- This is NOT machine learning (no training, no models)
- Simple feedback mechanism with exponentially weighted moving averages

### Performance Comparison

We rigorously compared 4 routing algorithms across 400 test runs:

1. **Standard Dijkstra** (baseline)
2. **Modified Dijkstra** (traffic + quality + vehicle aware)
3. **A* Search** (heuristic-guided)
4. **Adaptive Dijkstra** (feedback-learning - NOVEL)

#### Results Summary

| Algorithm | Avg Time (ms) | Nodes Explored | Improvement |
|-----------|---------------|----------------|-------------|
| Standard Dijkstra | 0.524 | 18.3 | Baseline |
| Modified Dijkstra | 0.612 | 16.7 | +8.7% efficiency |
| A* Search | 0.318 | 8.2 | **+55.2% efficiency** |
| Adaptive Dijkstra | 0.587 | 15.1 | +17.5% efficiency |

**Key Findings:**
- ✅ A* is fastest and most efficient (55% fewer nodes explored)
- ✅ Adaptive Dijkstra improves by 10% after learning from feedback
- ✅ All algorithms complete in <1ms (highly practical)
- ✅ Modified Dijkstra finds different paths considering real-world factors

#### Convergence Analysis

Adaptive Dijkstra demonstrates **learning behavior**:
- **Run 1:** 0.523 cost (no history)
- **Run 50:** 0.487 cost (with history) → **6.9% improvement**
- **Run 100:** 0.471 cost (with history) → **9.9% improvement**

### Experimental Methodology

**Test Environment:**
- Road network: 21 nodes, 34 edges (real Zanzibar locations)
- Test cases: 4 routes of varying length
- Runs per algorithm: 100 (for statistical significance)
- Hardware: Ubuntu 24, Python 3.12
...
Performance graphs available in `/experiments/results/`:
- `execution_time_comparison.png` - Bar chart of algorithm speeds
- `nodes_explored_comparison.png` - Efficiency comparison
- `efficiency_vs_distance.png` - Scalability analysis
- `cost_comparison.png` - Route quality comparison

### Running Experiments

```bash
# Run performance comparison (100 iterations per algorithm)
...
Complete experimental methodology, results, and analysis available in:
- **RESEARCH_REPORT.md** - Full research paper (12 pages)
